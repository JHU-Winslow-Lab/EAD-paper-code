...
void calc_fru_avg_local(double num_stat[11],
			...
			int LType_state_loc[MAX_LOCAL_NFRU][Nclefts_FRU][NLType_per_cleft][Nindepstates_LType],
  ...
  for(iFRU = 0;iFRU<NFRU_loc;iFRU++) {
    ...
    for(icleft = 0;icleft<Nclefts_FRU;icleft++) {
      ...
      for(i=0;i<NLType_per_cleft;i++) {
        if(LType_state_loc[iFRU][icleft][i][index_LCC_phosph]==1){
          if (LType_state_loc[iFRU][icleft][i][index_LCC_states]<=6) 
          {
            PNorm_Mode = PNorm_Mode + 1.0;
          }
          if (LType_state_loc[iFRU][icleft][i][index_LCC_Vinact]==Oy_LType) 
          {
            PnotVinact = PnotVinact + 1.0;
          }
          if (((LType_state_loc[iFRU][icleft][i][index_LCC_states]==O1_LType)||(LType_state_loc[iFRU][icleft][i][index_LCC_states]==O2_LType))
            &&(LType_state_loc[iFRU][icleft][i][index_LCC_Vinact]==Oy_LType)) 
          {
