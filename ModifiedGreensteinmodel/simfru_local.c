...
void simfru_local(const double time0,const double timef,
				  ...
				  int LType_state[Nclefts_FRU][NLType_per_cleft][Nindepstates_LType],
				  ...
				  unsigned long mt_loc[mtN+1])
{
	...
	double LType_trans_rates[Nclefts_FRU][NLType_per_cleft][4];
	double LType_trans_probs[Nclefts_FRU][NLType_per_cleft][4];
	int LType_index[Nclefts_FRU][NLType_per_cleft][4];
	int LType_length[Nclefts_FRU][NLType_per_cleft];
	double LType_Vinact_exitrate[Nclefts_FRU][NLType_per_cleft];
	double LType_Vinact_exitprob[Nclefts_FRU][NLType_per_cleft];
	...
	for(icleft = 0;icleft<Nclefts_FRU;icleft++) {
		...
		if ( ((LType_state[icleft][i][index_LCC_states]==O1_LType)||(LType_state[icleft][i][index_LCC_states]==O2_LType)) 
			&& (LType_state[icleft][i][index_LCC_Vinact]==Oy_LType)
			&& (LType_state[icleft][i][index_LCC_phosph]==1))  
		{
	...
	while (time_FRU<timef) {
		...
		for(icleft = 0;icleft<Nclefts_FRU;icleft++) {
			for(j=0;j<NLType_per_cleft;j++){
				// There are always at least two possible transitions
				LType_trans_probs[icleft][j][0] = 1.0 - time_stepFRU*LType_trans_rates[icleft][j][0];
				LType_trans_probs[icleft][j][1] = time_stepFRU*LType_trans_rates[icleft][j][1];
				for(i=2;i<LType_length[icleft][j];i++) {
					LType_trans_probs[icleft][j][i] = time_stepFRU*LType_trans_rates[icleft][j][i];
				}

				LType_Vinact_exitprob[icleft][j] = 1.0 - time_stepFRU*LType_Vinact_exitrate[icleft][j];
			}
		...
		for(icleft = 0;icleft<Nclefts_FRU;icleft++) {
			...
			for(i=0;i<NLType_per_cleft;i++){
				Accum_Prob = LType_trans_probs[icleft][i][0];
				if (unidev[base_ind]>Accum_Prob) {
					count = 1; // We know that state has been changed
					Accum_Prob += LType_trans_probs[icleft][i][1];
					while (unidev[base_ind]>Accum_Prob) {
						count++;
						Accum_Prob += LType_trans_probs[icleft][i][count];
					}

					LType_state[icleft][i][index_LCC_states] = LType_index[icleft][i][count];
				} // if

				if (unidev[1+base_ind]>=LType_Vinact_exitprob[icleft][i]) {
					LType_state[icleft][i][index_LCC_Vinact] = 3 - LType_state[icleft][i][index_LCC_Vinact];
				}
			}
