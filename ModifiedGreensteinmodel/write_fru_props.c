...
//wrong implementation on LType current recording.
//have to spend too much time on that.

void write_fru_props(FILE *filee[],FILE *file_avg[],FILE *file_avg_all,
		     const double time,
		     const double V,
		     const double Cai,
		     const double CaNSR,
		     ...
		     int Ito2_state[NFRU_sim_max][Nclefts_FRU])
{
  ...
  for(j = fru_start;j<=fru_end;j++) {
    ...
    for(jcleft = 0;jcleft<Nclefts_FRU;jcleft++) {
      ...
      JLType[jcleft] = 0.0;
      int iLCC = 0;
      if ( ((LType_state[j][index_LCC_states][iLCC][jcleft]==O1_LType)||(LType_state[j][index_LCC_states][iLCC][jcleft]==O2_LType))
	        && LType_state[j][index_LCC_Vinact][iLCC][jcleft]==Oy_LType) 
      {  
...
void open_fru_props(const int filenumber,FILE *filee[]) // Open output all files for local release unit data
{
  char filename[256];
  int j;
  ...
  for(j = fru_start;j<=fru_end;j++) {
    ...
    sprintf(filename,"%s%s%s_%d.%d.txt",output_dir,file_dir,output_fruprops_file,j,filenumber);
...
void open_fru_props_avg(const int filenumber,FILE *filee[]) // Open output all files for local release unit data
{
  char filename[256];
  int j;

  for(j = fru_start;j<=fru_end;j++) {
    sprintf(filename,"%s%s%s_%d.%d.txt",output_dir,file_dir,output_frupropsavg_file,j,filenumber);
    if ((filee[j-fru_start]=fopen(filename,"w+"))==NULL) {
      fprintf(stderr,"Cannot open file '%s'!\n",filename);
      exit(-5);
    }
...
FILE *open_fru_props_avgtot(const int filenumber) // Open output all files for local release unit data
{
  FILE *filee;
  char filename[256];

  sprintf(filename,"%s%s%s.%d.txt",output_dir,file_dir,output_frupropsavgtot_file,filenumber);
