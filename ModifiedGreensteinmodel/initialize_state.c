...
{
	double a1,a2;
	int k, iFRU, icleft,iLCC;
	FILE *ic_file;
	char filepath[256];

	int rewind_FRU;
	const int rewind_FRU_max=1500;
	...
	if (ic_file_flag) {
	  ...
	  for(iFRU=0;iFRU<NFRU_sim;iFRU++) {
		...
		//   for(iLCC=0;iLCC<NLType_per_cleft;iLCC++){
			...
			// 		LType_state[iFRU][icleft][iLCC][k]=read_next_int(ic_file);
			// 	}
		//   }
		// int LCC_state_tmp = read_next_int(ic_file);
		// int LCC_Vinact_tmp = read_next_int(ic_file);

		//------------Beta condition temporarily use--------------//
		for(iLCC=0;iLCC<NLType_per_cleft;iLCC++){
			LType_state[iFRU][icleft][iLCC][index_LCC_states]=read_next_int(ic_file);
			LType_state[iFRU][icleft][iLCC][index_LCC_Vinact]=read_next_int(ic_file);
			LType_state[iFRU][icleft][iLCC][index_LCC_phosph]=read_next_int(ic_file);
			LType_state[iFRU][icleft][iLCC][index_LCC_mode2] =read_next_int(ic_file);
			mode2count +=LType_state[iFRU][icleft][iLCC][index_LCC_mode2];

//			printf("%d ",mode2count);i
			// printf("%d %d %d %d \n",LType_state[iFRU][icleft][iLCC][index_LCC_states],
			// 		LType_state[iFRU][icleft][iLCC][index_LCC_Vinact],
			// 		LType_state[iFRU][icleft][iLCC][index_LCC_phosph],
			// 		LType_state[iFRU][icleft][iLCC][index_LCC_mode2]);
/*			
			LType_state[iFRU][icleft][iLCC][index_LCC_states]=1;
			LType_state[iFRU][icleft][iLCC][index_LCC_Vinact]=2;
			if (beta_flag){
				LType_state[iFRU][icleft][iLCC][index_LCC_phosph] = 1;
				//15% mode 2
				if((iFRU<NFRU_sim*.15) && (icleft<4)) {
					LType_state[iFRU][icleft][iLCC][index_LCC_mode2] = 1;
				}
				else{
					LType_state[iFRU][icleft][iLCC][index_LCC_mode2] = 0;
				}
			}
			else{
				// 40% 
				if(iFRU<NFRU_sim*.25/.6){
					//25% out of 60% LCC phosphorylated.
					LType_state[iFRU][icleft][iLCC][index_LCC_phosph] = 1;
				}
				else{
					LType_state[iFRU][icleft][iLCC][index_LCC_phosph] = 0;
				}
				LType_state[iFRU][icleft][iLCC][index_LCC_mode2] = 0;
			}
*/
			// if (iFRU<400){
			// 	if (iLCC<3){
			// 		LType_state[iFRU][icleft][iLCC][index_LCC_states]=LCC_state_tmp;
			// 		LType_state[iFRU][icleft][iLCC][index_LCC_Vinact]=LCC_Vinact_tmp;
			// 		LType_state[iFRU][icleft][iLCC][index_LCC_phosph] = 1;
			// 	}
			// 	else{
			// 		LType_state[iFRU][icleft][iLCC][index_LCC_phosph] = 0;							
			...
			// //15% mode 2 (actual in model is ~14.5%)
			// //1 of 8 in 600 FRUs with 2LCC/cleft and 2 of 12 in 400 FRUs with 3LCC/cleft
			// if(iFRU<400){
			// 	if((icleft==0) && (iLCC<2)){
			// 		LType_state[iFRU][icleft][iLCC][index_LCC_mode2] = 1;
			// 	}
		...
		//temporarily use for the previous initial state data
		//   for(iLCC=0;iLCC<NLType_per_cleft;iLCC++){
		// 	if(iLCC==0){
		// 		LType_state[iFRU][icleft][iLCC][index_LCC_states]=read_next_int(ic_file);
		// 		LType_state[iFRU][icleft][iLCC][index_LCC_Vinact]=read_next_int(ic_file);
		...
		// 	}
		// 	else{
		// 		LType_state[iFRU][icleft][iLCC][index_LCC_states]=0;
		// 		LType_state[iFRU][icleft][iLCC][index_LCC_Vinact]=0;
		// 		LType_state[iFRU][icleft][iLCC][index_LCC_phosph]=0;
		// 	}
		// 	LType_state[iFRU][icleft][iLCC][index_LCC_mode2]=0;
		//   }
...
//	  state[index_CaNSR] = 1.6;
//	  state[index_Cai] = 0.2e-3;
	  fclose(ic_file);
	...
	if (reset_RyRs_flag) {
		for(iFRU = 0;iFRU<NFRU_sim;iFRU++) {
			for(icleft = 0;icleft<Nclefts_FRU;icleft++) {			
				...
				LType_state[iFRU][icleft][k][index_LCC_states] = 1;
				LType_state[iFRU][icleft][k][index_LCC_Vinact] = Oy_LType;
				}
				for(k = 0;k<NRyRs_per_cleft;k++) {
					RyR_state[iFRU][icleft][k] = 1;
				}
