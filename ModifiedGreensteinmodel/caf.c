...
// LQTS1 mutation parameters definition
...
// #endif
  ...
  if (my_rank==0) {
    ...
    srand(time(NULL));
    filenumber = rand() % 10000000+1;
    initialize_ran(mt,mti,filenumber);
    ...
    // create folders for output saving
    mkdir(output_dir, 0755);
    char folderpath[256];
	  sprintf(folderpath,"%s%s",output_dir,file_dir);
    mkdir(folderpath, 0755);

    //read LQTS1 data
    Read_LQTS_data(setnumber);

    // open output files
    states_file=open_states(setnumber,filenumber);
    currents_file=open_currents(setnumber,filenumber);
    otherstates_file=open_otherstates(setnumber,filenumber);
