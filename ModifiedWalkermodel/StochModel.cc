...
StochModel::StochModel()
{
  ...
  //Initialization for LQTS-related parameters
  k_tau_plus_IKs = 1;
  k_tau_minus_IKs = 1;
  k_k_IKs = 1;
  k_Gmax_IKs = 1;
...
void StochModel::Read_LQTS1_p(std::string LQTS1_Data, int num)
{
...
void StochModel::Read_LQTS2_p(std::string LQTS2_Data, int num)
{
  int n = LQTS2_Data.length();
  char* char_LQTS2_Data = new char [n + 1];
  strcpy(char_LQTS2_Data, LQTS2_Data.c_str());

  int length;
  //Load Stoch data
  std::ifstream readerLQTS2;
  readerLQTS2.open(char_LQTS2_Data);
  readerLQTS2.seekg(0, std::ios::end);    // go to the end
  length = readerLQTS2.tellg();           // report location (this is the length)
  readerLQTS2.seekg(0, std::ios::beg);    // go back to the beginning
  char* buffer = new char[length];    // allocate memory for a buffer of appropriate dimension
  readerLQTS2.read(buffer, length);       // read the whole file into the buffer
  readerLQTS2.close();                    // close file handle
